...
def prev_prof_click():
    ...
    thread_safe_banner_update(this_result)
...
def thread_safe_banner_update(result):
    root.after(0, lambda: update_banner_text(result))
...
def t1_worker():
    ...
    while(1):
        ...
        thread_safe_banner_update(result)
...
if __name__ == "__main__":
    try:
        ...
        # Show info message if legacy numeric profiles detected
        if contains_jump_by_number():
            messagebox.showinfo("Info",
                                "Profiles are now referenced BY NAME (case sensitive) instead of number.\n\nMake sure to update the rules.")

        # 🔹 Start the background profile switch thread
        t1 = threading.Thread(target=t1_worker, daemon=True)
        t1.start()

        # 🔹 Start window title polling and main loop
        root.after(WINDOW_CHECK_FREQUENCY_MS, update_current_app_and_title)
        root.mainloop()
    except Exception as e:
        traceback.print_exc()
        messagebox.showerror("Startup Crash", str(e))
        sys.exit(1)
